...
void sendJsonToAWS()
{
  ...
  reportedObj["LAT"] = serialized(String(LAT,8));
  reportedObj["LONG"] = serialized(String(LONG,8));
  reportedObj["BatteryVoltage"] = batteryVoltage;
...
void sendCheck() {
  if (millis() - timer > 2000) {
    ...
    NFC();
    batteryVoltage = 2.00*3.30*(analogRead(A13)/4095.00);
    batteryVoltage = batteryVoltage*(4.20/3.99); //constant multiplier
